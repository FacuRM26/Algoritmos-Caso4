...
private:
    ...
    Point* cross(Point* parent_a, Point* parent_b) {
        ...
        unsigned int kidPositionX = (parent_a->getX() & mask_a) | (parent_b->getX() & mask_a);
        unsigned int kidPositionY = (parent_a->getY() & mask_b) | (parent_b->getY() & mask_b);
        // cout << "Corte:\n";
        ...
        Point* children = new Point(kidPositionX, kidPositionY, rand() % 255);
        return children;
    }
...
public:
    ...
    void initPopulation(int pAmountInitialPopulation) {
        ...
        for (int i = 0; i < pAmountInitialPopulation; i++) {
            ...
            population.push_back(new Point(randomX, randomY, rand() % 256));
        }
    };

    void fitness(int indexIndividual) {
