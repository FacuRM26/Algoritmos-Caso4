...
public:
    ...
    Probabilist(int width, int height, int sample, int maxPoints, int QuadrantSize, unsigned char* pImage)
    {
        ...
        for (int i = 0; i < sample; i++)
        {
            ...
            Quadrant *newQuadrant = new Quadrant(downLeft, upRight, make_pair(width, height), total / sample, gray);
    ...
    void incrementQuadrant(unsigned char *image, Quadrant *quadrant)
    {
        int count = 0;
        const size_t RGB = 3;
        while (count < 5)
